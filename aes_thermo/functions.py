...
def saturation_vapor_pressure_water(T,es_formula=es_default):
    ...
    formulations were found to be the most accurate as compared to the IAPWS standard for warm temperatures,
    ...
    def esla(T):
        c1 = (cpv-cl)/Rv
        c2 = lvT/(Rv*TvT) - c1
        return PvT * np.exp(c2*(1.-TvT/T)) * (T/TvT)**c1
...
def pseudo_theta_e(TK,PPa,qt,es_formula=es_default):
    ...
    es    = saturation_vapor_pressure_water
    p2r   = partial_pressure_to_mixing_ratio
    r2p   = mixing_ratio_to_partial_pressure
    
    rv = np.minimum(qt/(1.-qt), p2r(es(TK,es_formula),PPa))  # mixing ratio of vapor (not gas Rv)
    pv = r2p(rv,PPa)
...
def T_from_Te(Te,P,qt,es_formula=es_default):
    ...
    def zero(T,Te,P,qt):
        ...
        return  np.abs(Te-_theta_e(T,P,qt,es_formula))
...
def T_from_Tl(Tl,P,qt,es_formula=es_default):
    ...
    def zero(T,Tl,P,qt):
        ...
        return  np.abs(Tl-_theta_l(T,P,qt,es_formula))
...
def T_from_Ts(Ts,P,qt,es_formula=es_default):
    ...
    def zero(T,Ts,P,qt):
        ...
        return  np.abs(Ts-_theta_s(T,P,qt,es_formula))
